...
parser = argparse.ArgumentParser(
    description="Storing and processing Flashcards")

parser.add_argument("--import_from",
                    help="(optional) set the file to import Flashcards from")
parser.add_argument("--export_to",
                    help="(optional) set the file to import Flashcards to")
args = parser.parse_args()

# check if import_from argument was used
if args.import_from:
    ...
    if choice == 'add':
        ...
        for i in range(q):
            ...
            if answer == card.definition:
                ...
                if answer in [flashcard.definition for flashcard in flashcards]:
                    logging.info(
                        f"Wrong. The right answer is \"{card.definition}\", but your definition is correct for \""
                        f"{[flashcard.term for flashcard in flashcards][[flashcard.definition for flashcard in flashcards].index(answer)]}\".\n")
                else:
                    logging.info(f"Wrong. The right answer is \"{card.definition}\".\n")
